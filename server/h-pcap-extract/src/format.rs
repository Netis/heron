...
//! encode/decode used by `h-capture::pcap_dump`. Duplicated rather than
//! shared via a dependency so this read crate doesn't pull in libpcap /
//! ZMQ / snap-via-h-capture / retention. The on-disk format is the
//! actual contract; anchor tests below pin it.
//!
//! See also: `server/h-capture/src/pcap_dump.rs` (writer side).
...
/// Must match `h-capture::pcap_dump::PCAP_SNAPLEN`. Both sides re-anchor
...
mod tests {
    ...
    /// `h-capture::pcap_dump`'s own tests for the same constants.
    #[test]
    ...
    /// `h-capture::pcap_dump::tests::minute_label_format`.
    #[test]
