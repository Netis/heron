//! End-to-end: read pcap → h-protocol stage → h-llm stage →
//! h-turn tracker → assert turn counts against ground truth.
//!
...
async fn generic_profile_anthropic_two_call_session() {
    ...
    fn make_call(req: &str, resp: &str, ts_us: i64, finish: Option<&str>) -> LlmCall {
        LlmCall {
            source_id: "test".into(),
            id: format!("c-{ts_us}"),
            wire_api: h_llm::wire_apis::ANTHROPIC,
    ...
    let wire_apis = Arc::new(h_llm::wire_apis::build_default_wire_api_registry());
    ...
    let info1 = build_agent_call_info(
        &call1,
        &registry,
        &wire_apis,
        &h_llm::agent_classifier::ClassifierConfig::default(),
    ...
    let info2 = build_agent_call_info(
        &call2,
        &registry,
        &wire_apis,
        &h_llm::agent_classifier::ClassifierConfig::default(),
