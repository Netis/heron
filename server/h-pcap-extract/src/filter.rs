...
//! Records that don't decode to TCP via `h_protocol::de::decode` are
...
impl Filter<'_> {
    pub fn matches(&self, rec: &RawRec) -> bool {
        ...
        // h_protocol::de::decode wants a source_id + ts_us only for diagnostic
...
mod tests {
    ...
    fn drops_non_tcp_packet() {
        // Pure Ethernet, no IPv4: h_protocol::de::decode returns Err.
