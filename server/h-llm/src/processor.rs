...
impl LlmProcessor {
    ...
    fn on_exchange(
        ...
        let call = LlmCall {
            ...
            // Agent-classification fields are populated in the LLM stage
            // (stage.rs) once the agent match result is known.
...
/// `h-storage` and `app/dbview` — places that have an `LlmCall` reconstructed
/// from the database and no upstream parsed `Value` to hand in.
///
/// Public so tests in downstream crates (h-turn) can construct call-info
...
pub fn build_agent_call_info(
    ...
    classifier_cfg: &ClassifierConfig,
    metrics: &h_common::internal_metrics::MetricsWorker,
...
pub fn build_agent_call_info_with_parsed(
    ...
    let Some(ids) = profile.extract_session_id(&ctx) else {
        if is_generic {
            metrics
                .counter(h_common::internal_metrics::Metric::LlmGenericSessionIdSynthFailed)
                .inc();
        }
        return None;
    };
    if is_generic && ids.tool_id_canonicalized {
        metrics
            .counter(h_common::internal_metrics::Metric::LlmGenericToolIdCanonicalized)
            .inc();
    }
    ...
    if classified.tool_surface == Some(h_common::agent::ToolSurface::Unknown) {
        metrics
            .counter(h_common::internal_metrics::Metric::AgentClassifierUnknownCount)
            .inc();
    }
    if classified.tool_surface == Some(h_common::agent::ToolSurface::Mixed) {
        metrics
            .counter(h_common::internal_metrics::Metric::AgentClassifierMixedCount)
            .inc();
    }
