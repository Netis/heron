...
LOAD=0 DURATION=60 MAX_QUEUE_PCT=80 MAX_RSS_GROWTH_PCT=50 MIN_PKTS=10000
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    # Load/soak mode: loop the corpus for <duration> s and assert perf +
    # reliability invariants (drops, queue depth, RSS growth, throughput)
    # instead of single-pass correctness. Needs a binary with pcap-file loop
    # support (loop_secs).
    --load)      LOAD=1; shift;;
...
soak_one() {
  ...
  # In load mode, drive the binary's pcap-file loop replay for the window.
  local loop_line=""
  [ "$LOAD" = 1 ] && loop_line="loop_secs = $DURATION"
