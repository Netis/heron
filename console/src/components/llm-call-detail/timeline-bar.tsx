...
export function TimelineBar({ detail }: Props) {
  const { request_time, complete_time, ttft_ms, e2e_latency_ms } = detail
  ...
  const ttftRatio = (ttft_ms ?? 0) / e2e_latency_ms
  const genRatio = 1 - ttftRatio
    ...
    <div className="rounded-lg border border-border bg-muted/30 px-4 py-3">
      ...
      <div className="flex h-6 overflow-hidden rounded-md">
        ...
        {genRatio > 0 && (
          ...
          >
            Gen {formatMs(e2e_latency_ms - (ttft_ms ?? 0))}
          </div>
        )}
      </div>
