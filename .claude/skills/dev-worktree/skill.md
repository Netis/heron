...
Manage git worktrees for parallel development with multiple Claude Code sessions.
...
Claude Code has documented issues when operating across worktree boundaries:
- [#14652](https://github.com/anthropics/claude-code/issues/14652): Lock acquisition failures in multi-process scenarios
- [#12932](https://github.com/anthropics/claude-code/issues/12932): Worktree sync problems
- [#10105](https://github.com/anthropics/claude-code/issues/10105): Directory confusion

See Research #0006 for full analysis.

**Correct Workflow**:
```
Root Claude:  /dev-worktree add foo
              → prints: cd .worktrees/foo && claude

New Terminal: cd .worktrees/foo && claude
              → work, commit

Either:       /dev-worktree merge
              → cherry-pick to main

Root Claude:  /dev-worktree remove foo
              → remove worktree
```
...
just wt dev <name>       # Open claude in worktree
