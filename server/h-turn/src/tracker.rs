...
/// once at the h-llm boundary and carried on `AgentCall.agent`. The tracker
...
impl TurnTracker {
    ...
    pub fn ingest_at(&mut self, ic: AgentCall, now_wall: Instant) -> Vec<TurnEvent> {
        ...
        // protocol-terminal verdict from h-llm. Same composition pattern as
...
/// where the protocol-terminal field is set at h-llm time. Profiles whose
