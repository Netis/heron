...
mod tests {
    ...
    #[test]
    fn test_normalize_input_parses_request_body() {
        let ip: IpAddr = IpAddr::from([127, 0, 0, 1]);
        let body = serde_json::json!({
            "model": "claude-3",
        ...
        let req = ts_protocol::model::HttpRequestData {
            flow_key: FlowKey::new(String::new(), ip, 1234, ip, 8080),
            client_addr: (ip, 1234),
            server_addr: (ip, 8080),
        ...
        let v = AnthropicWireApi.normalize_input(&req);
        assert_eq!(v["model"], "claude-3");
        assert_eq!(v["messages"][0]["role"], "user");
    }

    #[test]
    fn test_normalize_output_non_sse_parses_response_body() {
        let ip: IpAddr = IpAddr::from([127, 0, 0, 1]);
        let body = serde_json::json!({
            "id": "msg_01",
            "model": "claude-3",
        ...
        let v = AnthropicWireApi.normalize_output(&resp, &[]);
        assert_eq!(v["id"], "msg_01");
        assert_eq!(v["content"][0]["text"], "hello");
    }

    #[test]
    fn test_normalize_output_sse_reconstructs_as_value() {
        let events = vec![
            make_sse(
                "message_start",
                r#"{"message":{"id":"msg_01","model":"claude-3","role":"assistant"}}"#,
            ),
            make_sse(
                "content_block_start",
                r#"{"index":0,"content_block":{"type":"text","text":""}}"#,
        ...
        // SSE path: resp body is empty (parser drops it), sse_events drive reconstruction.
        let ip: IpAddr = IpAddr::from([127, 0, 0, 1]);
        let resp = ts_protocol::model::HttpResponseData {
            flow_key: FlowKey::new(String::new(), ip, 1234, ip, 8080),
            client_addr: (ip, 1234),
        ...
        let v = AnthropicWireApi.normalize_output(&resp, &events);
