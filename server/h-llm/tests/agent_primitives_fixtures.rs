...
use h_llm::agent_primitives::{AgentPrimitives, SystemPromptMarkers};
use h_llm::profile::AgentProfile;
...
fn run<P: AgentProfile>(profile: &P, fixture: &str) -> AgentPrimitives {
    ...
    let llm_call = h_llm::test_support::empty_llm_call();
    let ctx = h_llm::profile::CallCtx::new(&llm_call, Some(&req), None);
...
fn opencode_mcp_tool_visible() {
    let p = run(&h_llm::agents::OpencodeProfile, "opencode_mcp.json");
    assert!(p.tool_names.iter().any(|n| n.starts_with("mcp__")));
}

#[test]
fn hermes_bash_tool_visible() {
    let p = run(&h_llm::agents::HermesProfile, "hermes_bash.json");
    assert!(p.tool_names.iter().any(|n| n == "bash"));
}

#[test]
fn generic_unknown_tool_visible() {
    let p = run(&h_llm::agents::GenericProfile, "generic_unknown_tool.json");
