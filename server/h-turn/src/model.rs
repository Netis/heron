...
///   [`ActiveTraceRegistry`]; never written to the `agent_turns` table.
...
impl fmt::Display for TraceStatus {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            TraceStatus::Incomplete => write!(f, "incomplete"),
        }
    }
}
...
/// entry when the turn finalizes. The `h-api` `/api/agent-turns` handler
...
pub type ActiveTraceRegistry = Arc<RwLock<HashMap<String, Trace>>>;

/// Construct an empty `ActiveTraceRegistry` ready to share between
/// trackers and the API.
pub fn new_active_trace_registry() -> ActiveTraceRegistry {
...
pub struct Trace {
    ...
    pub tool_surfaces: Vec<h_common::agent::ToolSurface>,
...
mod tests {
    ...
    #[test]
    fn status_display() {
        assert_eq!(TraceStatus::Complete.to_string(), "complete");
        assert_eq!(TraceStatus::Incomplete.to_string(), "incomplete");
    }
    ...
    fn agent_turn_has_span_ids() {
        let turn = Trace {
            ...
            final_answer_preview: None,
            final_call_id: None,
            span_ids: vec!["call-1".into(), "call-2".into()],
            metadata: serde_json::json!({}),
            tool_surfaces: vec![],
            tool_call_total: 0,
            agent_topology: None,
        ...
        assert_eq!(turn.span_ids.len(), 2);
        assert_eq!(turn.span_ids[0], "call-1");
    }
}
